...
@Component({
  selector: 'app-smr-rpt-today-deliveryreport',
  templateUrl: './smr-rpt-today-deliveryreport.component.html',
  styleUrls: ['./smr-rpt-today-deliveryreport.component.scss']
})
export class SmrRptTodayDeliveryreportComponent {
  task_list:any;
  MyenquiryCount_list :any;
  GetTodayDeliveryOrderReport_List :any;
  tax4_list : any;
  responsedata: any;
  data: any;  
  salesorder_list :any;
  ...
  ngOnInit(): void { 
    this.GetTodayDeliveryOrderReport ();
    this.GetDaySummary ();
    this.GetWeekSummary();
 ...
 {
  var url = 'SmrRptTodaysSalesReport/GetTodayDeliveryOrderReport'
  this.NgxSpinnerService.show()
  this.service.get(url).subscribe((result: any) => {
    $('#GetTodayDeliveryOrderReport_List').DataTable().destroy();
    this.responsedata = result;
    this.GetTodayDeliveryOrderReport_List = this.responsedata.GetTodayDeliveryOrderReport_List;
    setTimeout(() => {
      $('#GetTodayDeliveryOrderReport_List');
    }, 1);

...
@Component({
  selector: 'app-crm-trn-potentialmarketing',
  templateUrl: './crm-trn-potentialmarketing.component.html',
  styleUrls: ['./crm-trn-potentialmarketing.component.scss']
})
export class CrmTrnPotentialmarketingComponent {
  ...
  ngOnInit(): void {
    this.GetPotentialtManagerSummary();
...
//// Summary Grid//////
GetPotentialtManagerSummary() {
  this.NgxSpinnerService.show();
  var url = 'Marketingmanager/GetPotentialtManagerSummary'
  
  this.service.get(url).subscribe((result: any) => {
    $('#Potential').DataTable().destroy();
    this.responsedata = result;
    this.Potential = this.responsedata.Potential;

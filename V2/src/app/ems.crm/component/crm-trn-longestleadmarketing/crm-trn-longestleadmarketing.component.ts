...
@Component({
  selector: 'app-crm-trn-longestleadmarketing',
  templateUrl: './crm-trn-longestleadmarketing.component.html',
  styleUrls: ['./crm-trn-longestleadmarketing.component.scss']
})
export class CrmTrnLongestleadmarketingComponent {
  ...
  ngOnInit(): void {
    ...
    this.GetMarketingManagerLongestSummary();
    this.Getteamcount();
...
GetMarketingManagerLongestSummary(){
  this.NgxSpinnerService.show();
    var url = 'MarketingManager/GetMarketingManagerLongestSummary'
    this.service.get(url).subscribe((result: any) => {
      $('#totallist').DataTable().destroy();
      this.responsedata = result;
      this.totallist = this.responsedata.marketingmanager_longestlists;

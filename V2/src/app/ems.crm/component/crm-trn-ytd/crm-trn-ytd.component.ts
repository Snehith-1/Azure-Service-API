...
@Component({
  selector: 'app-crm-trn-ytd',
  templateUrl: './crm-trn-ytd.component.html',
  styleUrls: ['./crm-trn-ytd.component.scss']
})
export class CrmTrnYtdComponent {
...
GetTotallapsedlongest() {
  ...
  popmodal(parameter: string, parameter1: string) {
    ...
    this.customer_name = parameter1;
  }
  //360//
  Onopen(param1: any, param2: any) {
    const secretKey = 'storyboarderp';
    const lspage1 = "MM-ytd";
...
GetYTD(){
  this.NgxSpinnerService.show();
    var url = 'MarketingManager/GetYTD'
    this.service.get(url).subscribe((result: any) => {
      $('#M2D').DataTable().destroy();
      this.responsedata = result;
      this.M2D = this.responsedata.M2D;
...
openModallog3(parameter: string) {
  ...
  this.reactiveFormfollow.get("customer_name")?.setValue(this.parameterValue1.customer_name);
  this.customer_name = this.parameterValue1.customer_name;
}
//**Transfer log popup**//
openModallog4(parameter: string) {
  ...
  this.reactiveFormTransfer.get("customer_name")?.setValue(this.parameterValue1.customer_name);
  this.customer_name = this.parameterValue1.customer_name;
}

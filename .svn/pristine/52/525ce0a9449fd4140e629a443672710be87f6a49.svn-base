...
@Component({
  selector: 'app-crm-mst-product-edit',
  templateUrl: './crm-mst-product-edit.component.html',
  styleUrls: ['./crm-mst-product-edit.component.scss']
})
export class CrmMstProductEditComponent {
  isReadOnly = true;
  product!: IProduct;
  defaultAuth: any = { }; 
  product_gid:any;
     ...
     this.service.post(api, this.productform.value).subscribe(
      (result: any) => {
        ...
        if(result.status ==false){
          ...
          this.route.navigate(['/crm/CrmMstProductsummary']);

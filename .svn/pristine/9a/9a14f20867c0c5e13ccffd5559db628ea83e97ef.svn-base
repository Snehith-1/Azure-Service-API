...
@Component({
  selector: 'app-smr-rpt-today-paymentreport',
  templateUrl: './smr-rpt-today-paymentreport.component.html',
  styleUrls: ['./smr-rpt-today-paymentreport.component.scss']
})
export class SmrRptTodayPaymentreportComponent {
  task_list:any;
  MyenquiryCount_list :any;
  tax4_list : any;
  salesorder_list :any;
  GetMonthwiseOrderReport_List :any;
  GetTodayPaymentReport_List :any;
  responsedata: any;
  data: any; 
  today_total_so:any;
  today_total_do:any;
 ...
 {
  var url = 'SmrRptTodaysSalesReport/GetTodayPaymentReport'
  this.NgxSpinnerService.show()
  this.service.get(url).subscribe((result: any) => {
    $('#GetTodayPaymentReport_List').DataTable().destroy();
    this.responsedata = result;
    this.GetTodayPaymentReport_List = this.responsedata.GetTodayPaymentReport_List;
    setTimeout(() => {
      $('#GetTodayPaymentReport_List');
    }, 1);

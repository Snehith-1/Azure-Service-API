...
@Component({
  selector: 'app-crm-trn-customermarketing',
  templateUrl: './crm-trn-customermarketing.component.html',
  styleUrls: ['./crm-trn-customermarketing.component.scss']
})
export class CrmTrnCustomermarketingComponent {
  ...
  ngOnInit(): void {
    this.GetCustomerManagerSummary();
...
//// Summary Grid//////
GetCustomerManagerSummary() {
  this.NgxSpinnerService.show();
  var url = 'Marketingmanager/GetCustomerManagerSummary'
  this.service.get(url).subscribe((result: any) => {
    $('#Customer1').DataTable().destroy();
    this.responsedata = result;
    this.Customer1 = this.responsedata.Customer1;

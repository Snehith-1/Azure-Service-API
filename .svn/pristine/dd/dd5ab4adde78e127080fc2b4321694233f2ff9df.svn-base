    ...
    public class DaTelegram
    {
        ...
        public void DaGetTelegramdetails(telegramlist values)
        {
            try
            {
                ...
                {
                    values.telegram_gid = objMySqlDataReader["telegram_gid"].ToString();
                    values.id = objMySqlDataReader["id"].ToString();
                    values.title = objMySqlDataReader["page_name"].ToString();
                    values.invite_link = objMySqlDataReader["page_link"].ToString();
                    values.bot_name = objMySqlDataReader["bot_name"].ToString();
                }
        ...
        public void DaGetTelegramImage(MdlTelegram values)
        {
          ...
          "from crm_smm_ttelegram where upload_type='Video') as video_count,(select ifnull(count(upload_type),0)from crm_smm_ttelegram ) as total_count,(select ifnull(count(upload_type),0) from crm_smm_ttelegram where upload_type='Text') as text_count ;";
                ...
                if (objMySqlDataReader.HasRows)
                {
                    values.image_count = objMySqlDataReader["image_count"].ToString();
                    values.video_count = objMySqlDataReader["video_count"].ToString();
                    values.text_count = objMySqlDataReader["text_count"].ToString();
                    values.total_count = objMySqlDataReader["total_count"].ToString();
        ...
        public void DaTelegramrepostupload( telegramsummary_list values, string user_gid, result objResult)
        {
            try
            {
                ...
                if (objMySqlDataReader.HasRows)
                {
                    lsupload_path = objMySqlDataReader["upload_path"].ToString();
                    lsupload_type = objMySqlDataReader["upload_type"].ToString();
        ...
        public telegramconfiguration telegramcredentials()
        {
            ...
            if (objMySqlDataReader.HasRows == true)
            {
                ...
                gettelegramcredentials.bot_id = objMySqlDataReader["bot_id"].ToString();
                gettelegramcredentials.chat_id = objMySqlDataReader["chat_id"].ToString();

    ...
    public class DaRegisterLead
    {
        ...
        public void DaGetRegisterLeadSummary(MdlRegisterLead values, string employee_gid)
        {
            try
            {
               ...
               "where a.main_branch ='Y' and a.customer_type = 'Distributor'and a.created_by='" + employee_gid + "'" +
        ...
        public void DaGetRegisterLeadSummary1(MdlRegisterLead values, string employee_gid)
        {
            try
            {
                ...
                "left join hrm_mst_temployee h on  g.assign_to = h.employee_gid  " +
                "left join adm_mst_tuser j on h.user_gid = j.user_gid " +
                "where a.main_branch ='Y' and a.customer_type = 'Corporate'and a.created_by='" + employee_gid + "'" +
                "group by a.leadbank_gid Order by date(a.created_date) desc,a.created_date asc,a.leadbank_gid desc";
        ...
        public void DaGetRegisterLeadSummary2(MdlRegisterLead values, string employee_gid)
        {
            try
            {
                ...
                "left join hrm_mst_temployee h on  g.assign_to = h.employee_gid  " +
                "left join adm_mst_tuser j on h.user_gid = j.user_gid " +
                "where a.main_branch ='Y' and a.customer_type = 'Retailer'and a.created_by='" + employee_gid + "'" +
                "group by a.leadbank_gid Order by date(a.created_date) desc,a.created_date asc,a.leadbank_gid desc";
        ...
        public void DaPostregisterlead(string employee_gid, Registerlead_list values)
        {
            try
            {
                ...
                if (mnResult == 1)
                {
                    msSQL = "select mobile from crm_trn_tleadbankcontact where leadbank_gid = '" + msGetGid1 + "' and leadbankcontact_gid='" + msconGetGID + "'";
                    objMySqlDataReader = objdbconn.GetDataReader(msSQL);
                    if (objMySqlDataReader.HasRows)
                    {
                        string mobile = objMySqlDataReader["mobile"].ToString();
                        ...
                        if (objMySqlDataReader.HasRows)
                        {
                            string leadbank_name = objMySqlDataReader["leadbank_name"].ToString();
                            string firstName = objMySqlDataReader["firstName"].ToString();
                            string lastName = objMySqlDataReader["lastName"].ToString();
     ...
     " where a.leadbank_gid ='" + leadbank_gid + "'";
        ...
        public void DaGetRegisterLeadCount(string employee_gid, string user_gid, MdlRegisterLead values)
        {
            try
            {
                ...
                msSQL = " select (select count(leadbank_gid) from crm_trn_tleadbank where customer_type='Distributor' and " +
           " created_by='" + employee_gid + "') as distributor_count, " +
           " (select count(leadbank_gid) from crm_trn_tleadbank where customer_type='Retailer' and " +
           " created_by='" + employee_gid + "') as retailer_counts," +
           " (select count(leadbank_gid) from crm_trn_tleadbank where customer_type='Corporate' and " +
           " created_by='" + employee_gid + "') as corporate_count, " +
           " (select count(leadbank_gid) from crm_trn_tleadbank where " +

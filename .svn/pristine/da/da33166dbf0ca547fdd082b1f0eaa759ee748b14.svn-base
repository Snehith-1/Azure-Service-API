...
" where a.created_by='" + user_gid + "'";
        ...
        public void DaGetPurchaseRequisitionView(string purchaserequisition_gid, MdlPmrTrnPurchaseRequisition values)
        {
            try
            {
                ...
                msSQL = " select distinct a.purchaserequisition_gid,a.priority_remarks, " +
" DATE_FORMAT(a.purchaserequisition_date,'%d-%m-%Y') as purchaserequisition_date, a.purchaserequisition_flag,replace(a.purchaserequisition_remarks,'PI','PR') as purchaserequisition_remarks, " +
" b.user_firstname,d.department_name,CONCAT_WS('/', e.branch_name, d.department_name) AS branch_name," +
" replace(a.purchaserequisition_referencenumber,'PI','PR') as purchaserequisition_referencenumber,h.product_gid, " +
" h.product_name,h.product_code,h.productgroup_name,h.productuom_name,h.qty_requested,h.display_field" +
" from pmr_trn_tpurchaserequisition a " +
" left join adm_mst_tuser b on a.created_by = b.user_gid " +
" left join hrm_mst_temployee c on c.user_gid = b.user_gid " +
" left join hrm_mst_tdepartment d on c.department_gid = d.department_gid " +
" left join hrm_mst_tbranch e on a.branch_gid = e.branch_gid " +
" left join adm_mst_tmodule2employee  f on f.employee_gid = c.employee_gid " +
" left join pmr_trn_tpurchaserequisitiondtl h on a.purchaserequisition_gid = h.purchaserequisition_gid " +
" where  a.purchaserequisition_gid= '" + purchaserequisition_gid + "' group by a.purchaserequisition_gid, h.product_gid";
                dt_datatable = objdbconn.GetDataTable(msSQL);

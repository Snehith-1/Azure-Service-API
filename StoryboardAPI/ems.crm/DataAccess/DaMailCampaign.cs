    ...
    public class DaMailCampaign
    {
        ...
        public void DaGetMailSummary(MdlMailCampaign values)
        {
            ...
            { 
                ...
                msSQL = " select a.mailmanagement_gid,a.from_mail,a.to_mail,a.direction," +
                    ...
                    "left join temp_crm_smm_mailmanagement b on a.temp_mail_gid=b.temp_mail_gid order by or_date desc; ";
...

        {
            ...
            try
            {
                ...
                var body = "{\"options\":{\"open_tracking\":true,\"click_tracking\":true},\"recipients\":[{\"address\":{\"email\":" + "\"" + values.to_mail.Replace(" ","") + "\"" + "}},{\"address\":{\"email\":" + "\"" + values.cc + "\"" + ",\"header_to\":" + "\"" + values.to_mail.Replace(" ", "") + "\"" + "}},{\"address\":{\"email\":" + "\"" + values.bcc + "\"" + ",\"header_to\":" + "\"" + values.to_mail.Replace(" ", "") + "\"" + "}}],\"content\":{\"from\":" + "\"" + getmailconfiguration.email_username +"<"+ getmailconfiguration.sending_domain + ">\"" + ",\"headers\":{\"CC\":" + "\"" + values.cc + "\"" + "},\"subject\":" + "\"" + values.sub + "\"" + ",\"reply_to\":" + "\"" + getmailconfiguration.receiving_domain + "\"" + ",\"html\":" + "\"" + values.body.Replace("\"", "\\\"").Replace("'", "\\\'").Replace("<img ", "<img style=\\\"width: 500px\\\"") + "\"" + "}}";
        ...
        public void DaMailUpload(HttpRequest httpRequest, string user_gid, results objResult)
        {
            ...
            List<DbAttachmentPath> dbattachmentpath = new List<DbAttachmentPath>();
                ...
                if (httpRequest.Files.Count > 0)
                {
                    ...
                    request.AddHeader("Authorization", ""+getmailconfiguration.access_token+"");
                    request.AddHeader("Content-Type", "application/json");
                    var bodies = "{\"options\":{\"open_tracking\":true,\"click_tracking\":true},\"recipients\":[{\"address\":{\"email\":" + "\"" + to + "\"" + "}},{\"address\":{\"email\":" + "\"" + cc + "\"" + ",\"header_to\":" + "\"" + to + "\"" + "}},{\"address\":{\"email\":" + "\"" + bcc + "\"" + ",\"header_to\":" + "\"" + to + "\"" + "}}],\"content\":{\"from\":" + "\"" + getmailconfiguration.email_username + "<" + getmailconfiguration.sending_domain + ">\"" + ",\"headers\":{\"CC\":" + "\"" + cc + "\"" + "},\"subject\":" + "\"" + sub + "\"" + ",\"reply_to\":" + "\"" + getmailconfiguration.receiving_domain + "\"" + ",\"html\":" + "\"" + body.Replace("\"", "\\\"").Replace("'", "\\\'").Replace("<img ", "<img style=\\\"width: 500px\\\"") + "\"" + "," + contentWithAttachments +"}}";
                    var body_content = JsonConvert.DeserializeObject(bodies);
                    request.AddParameter("application/json", body_content, ParameterType.RequestBody);
                    IRestResponse response = client.Execute(request);
                    string errornetsuiteJSON = response.Content;
        ...
        public void DaSaveTemplate(mailsummary_list values, string user_gid)
        {
            ...
            {
                    objcmnfunctions.LogForAudit("*******Date*****" + DateTime.Now.ToString("yyyy - MM - dd HH: mm:ss") + "***********" + $"DataAccess:{System.Reflection.MethodBase.GetCurrentMethod().Name}" + " * **********" + bytes.ToString() + "***********" + values.message.ToString() + "*****Query****" + msSQL + "*******Apiref********", "SocialMedia/ErrorLog/Mail/" + "Log" + DateTime.Now.ToString("yyyy - MM - dd HH") + ".txt");
                }
            ...
            var body = "{\"id\":" + "\"" + msGetGid1 + "\"" + ",\"name\":" + "\"" + values.template_name + "\"" + ",\"published\":true,\"options\":{\"open_tracking\":true,\"click_tracking\":true},\"content\":{\"from\":{\"email\":" + "\"" +getmailconfiguration.sending_domain+ "\"" + ",\"name\":" + "\"" + getmailconfiguration.email_username + "\"" + "},\"subject\":" + "\"" + values.sub + "\"" + ",\"reply_to\":" + "\"" +getmailconfiguration.receiving_domain + "\"" + ",\"html\":" + "\"" + temp_body + "\"" + "}}";
            var body_content = JsonConvert.DeserializeObject(body);
            request.AddParameter("application/json", body_content, ParameterType.RequestBody);
            IRestResponse response = client.Execute(request);
            string errornetsuiteJSON = response.Content;
        ...
        public void DaSendTemplate(mailtemplatesendsummary_list values, string user_gid)
        {
            try
            {
                ...
                for (int i = 0; i < values.mailsendchecklist.ToArray().Length; i++)
                {
                    ...
                    request.AddHeader("Content-Type", "application/json");
                    var body = "{\"options\":{\"open_tracking\":true,\"click_tracking\":true},\"content\":{\"template_id\":" + "\"" + values.temp_mail_gid + "\"" + "},\"recipients\":[{\"address\":{\"email\":" + "\"" + values.mailsendchecklist[i].email + "\"" + "}}]}";
                    ...
                    if (response.StatusCode == HttpStatusCode.OK)
                    {
                        ...
                        msSQL = "INSERT INTO crm_smm_mailmanagement (" +
                            ...
                            "VALUES (" +
                            "'" + msGetGid + "', " +
                               "'" + values.mailsendchecklist[i].leadbank_gid + "', " +
                             "'" + values.temp_mail_gid + "', " +
                            "'" + values.mailsendchecklist[i].email + "', " +
                            "'" + objMdlMailCampaignResponse.results.id + "', " +
                              "'" + user_gid + "'," +
                            "'" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "')";
                        mnResult = objdbconn.ExecuteNonQuerySQL(msSQL);
            ...
            {
                values.message = "Error While Sending Mail Template";
                objcmnfunctions.LogForAudit("*******Date*****" + DateTime.Now.ToString("yyyy - MM - dd HH: mm:ss") + "***********" + $"DataAccess:{System.Reflection.MethodBase.GetCurrentMethod().Name}" + " * **********" + ex.ToString() + "***********" + values.message.ToString() + "*****Query****" + msSQL + "*******Apiref********", "SocialMedia/ErrorLog/Mail/" + "Log" + DateTime.Now.ToString("yyyy - MM - dd HH") + ".txt");
...

        {
            try
            {
                ...
                msSQL = "select b.address1,b.address2, b.city,b.state,a.customer_type,b.leadbankcontact_name,b.email,b.mobile,a.leadbank_gid,a.source_gid,c.source_name,a.lead_status, d.template_name," +
                    "b.created_date from crm_trn_tleadbank a left join crm_trn_tleadbankcontact b on a.leadbank_gid=b.leadbank_gid left join crm_mst_tsource c on a.source_gid=c.source_gid left join temp_crm_smm_mailmanagement d on d.temp_mail_gid='" + temp_mail_gid + "';";
                dt_datatable = objdbconn.GetDataTable(msSQL);
...

        {
            try
            {
                ...
                msSQL = "Select body,template_name,sub,created_date from temp_crm_smm_mailmanagement where temp_mail_gid= '" + temp_mail_gid + "'";
                dt_datatable = objdbconn.GetDataTable(msSQL);
                var getmodulelist = new List<mailtemplate_list>();
                if (dt_datatable.Rows.Count != 0)
                {
        ...
        public mailconfiguration mailcrendentials()
        {
            ...
            try { 
                ...
                if (objMySqlDataReader.HasRows == true)
                {
                    ...
                    getmailcredentials.base_url = objMySqlDataReader["base_url"].ToString();
                    getmailcredentials.access_token = objMySqlDataReader["access_token"].ToString();
                    getmailcredentials.receiving_domain = objMySqlDataReader["receiving_domain"].ToString();
                    getmailcredentials.sending_domain = objMySqlDataReader["sending_domain"].ToString();
                    getmailcredentials.email_username = objMySqlDataReader["email_username"].ToString();

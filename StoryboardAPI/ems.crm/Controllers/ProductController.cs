...
namespace ems.crm.Controllers
{
    ...
    public class ProductController : ApiController
    {
        ...
        [HttpGet]
        public HttpResponseMessage GetShopifyProductdetails()
        {
            getproduct values = new getproduct();
            string token = Request.Headers.GetValues("Authorization").FirstOrDefault();
            getsessionvalues = objgetgid.gettokenvalues(token);
            values = objdaproduct.DaGetShopifyProductdetails(getsessionvalues.user_gid);
            return Request.CreateResponse(HttpStatusCode.OK, values);
        }
        ...
        public HttpResponseMessage Sendproductmaster(shopifyproductmove_list values)
        {
            string token = Request.Headers.GetValues("Authorization").FirstOrDefault();
            getsessionvalues = objgetgid.gettokenvalues(token);
            objdaproduct.DaSendproductmaster(getsessionvalues.user_gid, values);

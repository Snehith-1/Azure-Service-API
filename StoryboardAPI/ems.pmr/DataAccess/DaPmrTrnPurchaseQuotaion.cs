...
namespace ems.pmr.DataAccess
{
    public class DaPmrTrnPurchaseQuotaion
    {
        ...
        public void DaPostDirectQuotation(string employee_gid, post_list values)
        {
            ...
            {
                values.message = "Exception occured while inwarding grn!";
                objcmnfunctions.LogForAudit("*******Date*****" + DateTime.Now.ToString("yyyy - MM - dd HH: mm:ss") + "***********" +
             $"DataAccess: {System.Reflection.MethodBase.GetCurrentMethod().Name}" + "***********" +
             ex.Message.ToString() + "***********" + values.message.ToString() + "*****Query****" +
             msSQL + "*******Apiref********", "ErrorLog/Purchase/" + "Log" +
    ...
    " where quotationdtl_gid='" + quotationdtl_gid + "'";
        ...
        public void DaGetTermsandConditions(MdlPmrTrnPurchaseQuotation values)
        {
            try
            {
                ...
                msSQL = "  select c.termsconditions_gid, c.template_name,c.template_content,c.payment_terms  from pmr_trn_ttermsconditions c ";
                dt_datatable = objdbconn.GetDataTable(msSQL);

...
namespace ems.sales.DataAccess
{
    ...
    public class DaSmrRptTodaysSalesReport
    {
        ...
        public void DaGetDaySalesReportCount(string employee_gid, MdlSmrRptTodaysSalesReport values)
        {
            ...
            msSQL = "  select ifnull(format(sum(invoice_amount*exchange_rate),2),'0') as invoice_amount from rbl_trn_tinvoice " +
                   " where invoice_date='" + DateTime.Now.ToString("yyyy-MM-dd") + "' and invoice_from='Sales'";
            values.today_invoice_amount = objdbconn.GetExecuteScalar(msSQL);

            msSQL = " select ifnull(format(sum(amount*exchange_rate),2),'0') as payment_amount from rbl_trn_tpayment " +
                 " where payment_date='" + DateTime.Now.ToString("yyyy-MM-dd") + "'";
            values.today_payment_amount = objdbconn.GetExecuteScalar(msSQL);

            msSQL = " select ifnull(format((sum(invoice_amount*exchange_rate)-sum(payment_amount*exchange_rate)),2),'0') as outstanding_amount from rbl_trn_tinvoice " +
                 " where invoice_date='" + DateTime.Now.ToString("yyyy-MM-dd") + "' and invoice_from='Sales'";
        ...
        public void DaGetWeekSalesReportCount(string employee_gid, MdlSmrRptTodaysSalesReport values)
        {
            ...
            DateTime currStartDate = today.AddDays(-offset);
            DateTime currEndDate = today.AddDays(6 - offset);

            msSQL = " SELECT count(salesorder_gid) as total_so FROM smr_trn_tsalesorder " +
                    " where salesorder_date>='" + currStartDate.ToString("yyyy-MM-dd") + "'" +
                  " and salesorder_date<='" + currEndDate.ToString("yyyy-MM-dd") + "'  and so_type='Sales' ";
            values.cw_total_so = objdbconn.GetExecuteScalar(msSQL);

            msSQL = " SELECT count(directorder_gid) as total_do FROM smr_trn_tdeliveryorder " +
                    " where directorder_date>='" + currStartDate.ToString("yyyy-MM-dd") + "'" +
                  " and directorder_date<='" + currEndDate.ToString("yyyy-MM-dd") + "' ";
            values.cw_total_do = objdbconn.GetExecuteScalar(msSQL);

            msSQL = " SELECT count(invoice_gid) as total_invoice FROM rbl_trn_tinvoice " +
                    " where invoice_date>='" + currStartDate.ToString("yyyy-MM-dd") + "'" +
                  " and invoice_date<='" + currEndDate.ToString("yyyy-MM-dd") + "'  and invoice_from='Sales'";
            values.cw_total_invoice = objdbconn.GetExecuteScalar(msSQL);

            msSQL = "  SELECT count(payment_gid) as total_payment FROM rbl_trn_tpayment " +
                   " where payment_date>='" + currStartDate.ToString("yyyy-MM-dd") + "'" +
                  " and payment_date<='" + currEndDate.ToString("yyyy-MM-dd") + "'  ";
            ...
            DateTime lastStartDate = today.AddDays(-7 + offset_l);
            DateTime lastEndDate = today.AddDays(-1 + offset_l);

            msSQL = " SELECT count(salesorder_gid) as total_so FROM smr_trn_tsalesorder " +
                   " where salesorder_date>='" + lastStartDate.ToString("yyyy-MM-dd") + "'" +
                 " and salesorder_date<='" + lastEndDate.ToString("yyyy-MM-dd") + "'  and so_type='Sales' ";
            values.lw_total_so = objdbconn.GetExecuteScalar(msSQL);

            msSQL = " SELECT count(directorder_gid) as total_do FROM smr_trn_tdeliveryorder " +
                    " where directorder_date>='" + lastStartDate.ToString("yyyy-MM-dd") + "'" +
                  " and directorder_date<='" + lastEndDate.ToString("yyyy-MM-dd") + "' ";
            values.lw_total_do = objdbconn.GetExecuteScalar(msSQL);

            msSQL = " SELECT count(invoice_gid) as total_invoice FROM rbl_trn_tinvoice " +
                    " where invoice_date>='" + lastStartDate.ToString("yyyy-MM-dd") + "'" +
                  " and invoice_date<='" + lastEndDate.ToString("yyyy-MM-dd") + "'  and invoice_from='Sales'";
            values.lw_total_invoice = objdbconn.GetExecuteScalar(msSQL);

            msSQL = "  SELECT count(payment_gid) as total_payment FROM rbl_trn_tpayment " +
                   " where payment_date>='" + lastStartDate.ToString("yyyy-MM-dd") + "'" +
                  " and payment_date<='" + lastEndDate.ToString("yyyy-MM-dd") + "'  ";
            ...
            {
                values.message = "Exception occured while loading Week Sales Report Count !";
                objcmnfunctions.LogForAudit("*******Date*****" + DateTime.Now.ToString("yyyy - MM - dd HH: mm:ss") + "***********" +
                $"DataAccess: {System.Reflection.MethodBase.GetCurrentMethod().Name}" + "***********" + ex.Message.ToString() + "***********" +
                values.message.ToString() + "*****Query****" + msSQL + "*******Apiref********", "ErrorLog/Sales/" + "Log" + DateTime.Now.ToString("yyyy-MM-dd HH") + ".txt");
        ...
        public void DaGetMonthSalesReportCount(string employee_gid, MdlSmrRptTodaysSalesReport values)
        {
            ...
            msSQL = " SELECT count(salesorder_gid) as total_so FROM smr_trn_tsalesorder " +
                  " where salesorder_date>='" + currMonthStart.ToString("yyyy-MM-dd") + "'" +
                " and salesorder_date<='" + currMonthEnd.ToString("yyyy-MM-dd") + "'  and so_type='Sales' ";
            values.cm_total_so = objdbconn.GetExecuteScalar(msSQL);

            msSQL = " SELECT count(directorder_gid) as total_do FROM smr_trn_tdeliveryorder " +
                    " where directorder_date>='" + currMonthStart.ToString("yyyy-MM-dd") + "'" +
                  " and directorder_date<='" + currMonthEnd.ToString("yyyy-MM-dd") + "' ";
            values.cm_total_do = objdbconn.GetExecuteScalar(msSQL);

            msSQL = " SELECT count(invoice_gid) as total_invoice FROM rbl_trn_tinvoice " +
                    " where invoice_date>='" + currMonthStart.ToString("yyyy-MM-dd") + "'" +
            ...
            msSQL = "  SELECT count(payment_gid) as total_payment FROM rbl_trn_tpayment " +
                   " where payment_date>='" + currMonthStart.ToString("yyyy-MM-dd") + "'" +
            ...
            msSQL = " SELECT count(salesorder_gid) as total_so FROM smr_trn_tsalesorder " +
                  " where salesorder_date>='" + firstDayLastMonth.ToString("yyyy-MM-dd") + "'" +
            ...
            msSQL = " SELECT count(directorder_gid) as total_do FROM smr_trn_tdeliveryorder " +
                    " where directorder_date>='" + firstDayLastMonth.ToString("yyyy-MM-dd") + "'" +
                  " and directorder_date<='" + lastDayLastMonth.ToString("yyyy-MM-dd") + "' ";
            values.lm_total_do = objdbconn.GetExecuteScalar(msSQL);

            msSQL = " SELECT count(invoice_gid) as total_invoice FROM rbl_trn_tinvoice " +
                    " where invoice_date>='" + firstDayLastMonth.ToString("yyyy-MM-dd") + "'" +
            ...
            msSQL = "  SELECT count(payment_gid) as total_payment FROM rbl_trn_tpayment " +
                   " where payment_date>='" + firstDayLastMonth.ToString("yyyy-MM-dd") + "'" +
        ...
        public void DaGetYearSalesReportCount(string employee_gid, MdlSmrRptTodaysSalesReport values)
        {
            ...
            DateTime previousYearStartDate = new DateTime(currentDate.Year - 1, 1, 1);

            // Get the end date of the previous year
            DateTime previousYearEndDate = new DateTime(currentDate.Year - 1, 12, 31);
            ...
            {
                values.message = "Exception occured while Yearly Sales Order Report Count !";
                objcmnfunctions.LogForAudit("*******Date*****" + DateTime.Now.ToString("yyyy - MM - dd HH: mm:ss") + "***********" +
                $"DataAccess: {System.Reflection.MethodBase.GetCurrentMethod().Name}" + "***********" + ex.Message.ToString() + "***********" +
                values.message.ToString() + "*****Query****" + msSQL + "*******Apiref********", "ErrorLog/Sales/" + "Log" + DateTime.Now.ToString("yyyy-MM-dd HH") + ".txt");
